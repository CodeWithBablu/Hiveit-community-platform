...
import { DocumentReference, Timestamp, addDoc, collection, serverTimestamp, updateDoc } from "firebase/firestore";
...
const PostForm = ({ user }: { user: User }) => {
  ...
  const fetchMetadata = async (url: string) => {
    ...
    try {
      res = await axios.post(`https://metaminion.vercel.app/fetch-metadata/`, { url });
  ...
  const handleSubmit = async () => {
    ...
    if (payload.type === 'media' && filesState.files.length === 0) {
      return Toast('info-bottom', "please upload images/video to make post shine", 4000);
    }
    ...
    try {
      ...
      Toast('success', 'Your post is live! Time to watch the upvotes roll in! 🎉🎉', 5000);
      setLoading(false);
      ...
      return Toast('error-bottom', error instanceof FirebaseError
        ? FIREBASE_ERRORS[error.message as keyof typeof FIREBASE_ERRORS]
        : "failed to create post. Try again later 😭😞", 4000);
    }

  }

  const uploadFiles = async (postDocRef: DocumentReference, files: FileWithUrl[]) => {
    ...
    if (gallery.length > 0) {
      ...
      Toast('error-bottom', 'failed to create post. Try again later 😭😞', 4000);
  }
  ...
  return (
    <div className="mb-10 flex flex-col gap-2 px-2">
      <h1 className="mb-3 h-full w-fit bg-gradient-to-r from-gray-300 from-30% to-rose-500 to-80% bg-clip-text font-chillax text-2xl font-medium text-transparent">
        Create a post
      </h1>

      <div className="wrapper flex h-full flex-col gap-3 rounded-xl border-[1px] border-blue-900/60 px-2 py-2 sm:px-3">
        ...
        <div className="flex h-full flex-col gap-4 px-2 py-4 sm:py-6">
          ...
          {payload.type === "link" && <Link link={payload.link || ""} setPayload={setPayload} />}
        ...
        <div className="relative flex items-center self-end">
          <motion.button
            ...
            type="submit"
            className={clsx(
              `rounded-xl px-8 py-3 text-lg transition-all duration-300 font-medium`,
              {
                "pointer-events-none bg-gray-900/80 text-gray-400": !title || loading,
